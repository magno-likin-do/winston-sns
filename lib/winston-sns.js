  ...
  module.exports = winston.transports.SNS = (function(_super) {
    ...
    SNS.prototype.log = function(level, msg, meta, callback) {
      ...
      sub = function(str) {
        ...
        m = json ? JSON.stringify(meta.stack) : util.inspect(meta.stack, false, 10);
        return str.replace('%l', level).replace('%e', msg).replace('%m', m).replace(/\\n/g,"\n");
      };
      snsOpts = {
        Subject: sub(this.options.subject).substring(0, 99),
